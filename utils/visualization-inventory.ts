...
const VISUALIZATION_INVENTORY: readonly VisualizationFieldDefinition[] = [
  ...
  field({
    ...
    rationale: 'Superseded on the Today page by activitySummary, which merges topActivity and motionContext into one card.',
  }),
  field({
    key: 'activitySummary',
    label: 'Activity',
    kind: 'aggregate',
    sourceKeys: ['DailyAggregateRecord.topActivity', 'activity', 'motion_context'],
    primaryGroup: 'movement',
    surfaces: ['today', 'detail'],
    interactions: ['tap_card'],
    phaseOneVisibility: 'ship_now',
    description: 'Unified activity card combining top activity and movement quality from accelerometer data.',
    rationale: 'Replaces separate topActivity and motionContext cards on Today to reduce redundancy.',
  }),
  field({
