...
function isCollectorReady(collectors: CollectorStateMap, key: CollectorKey): boolean {
  const collector = collectors[key];
  return collector.enabled && (collector.permissionStatus === 'granted' || collector.permissionStatus === 'derived');
}
...
function buildDailyAggregate(date: string, index: number): DailyAggregateRecord {
  return {
    date,
    ...
    Object.entries(collectors).map(([key, collector]) => [
      key,
      {
        ...
        permissionStatus: collector.enabled
          ? (collector.key === 'sleep' ? 'derived' : 'granted')
          : 'not_requested',
        health: collector.enabled ? (collector.key === 'appUsage' ? 'degraded' : 'healthy') : 'idle',
        lastRunLabel: collector.enabled
          ? (collector.key === 'appUsage' ? '18m ago' : collector.key === 'sleep' ? 'This morning' : 'Moments ago')
          : 'Off',
      },
    ]),
...
export function hasAnyEnabledCollectors(collectors: CollectorStateMap): boolean {
...
export function buildDashboardMetrics(
  ...
  return [
    createMetric(stepsReady, 'Steps', formatNumber(8472), 'Daily baseline from phone sensors.', 'hero', 'steps'),
    createMetric(activityReady, 'Active Minutes', '74', 'Moderate movement across the day.', 'physical', 'activeMinutes'),
    createMetric(usageReady, 'Screen Time', formatMinutes(168), 'Usage access is partially degraded.', 'cool', 'screenTime'),
    createMetric(locationReady, 'Mobility Radius', '6.4 km', 'Derived from optional location history.', 'human', 'mobilityRadius'),
  ];
}
...
export function buildTrendSeries(
  ...
  const definitions = [
    { key: 'steps', label: 'Steps', unit: 'count', tone: 'hero' as const, min: 6200, max: 11800, phase: 1, ready: isCollectorReady(collectors, 'steps') },
    { key: 'active', label: 'Active Minutes', unit: 'min', tone: 'physical' as const, min: 36, max: 96, phase: 2, ready: isCollectorReady(collectors, 'activity') },
    { key: 'screen', label: 'Screen Time', unit: 'min', tone: 'cool' as const, min: 104, max: 246, phase: 4, ready: isCollectorReady(collectors, 'appUsage') },
  ];
  ...
  return definitions
    ...
    .map((definition) => ({
      ...
      points: labels.map((label, index) => ({
        label,
        value: buildWaveValue(index, definition.min, definition.max, definition.phase),
      })),
...
export function buildDailyAggregates(range: TrendRange): DailyAggregateRecord[] {
...
export function buildExportEvents(
  ...
  if (isCollectorReady(collectors, 'steps')) {
    exports.steps = Array.from({ length: 6 }, (_, index) => createEventRecord('steps', 'sensor', index, {
  ...
  if (isCollectorReady(collectors, 'activity')) {
    const activityTypes = ['walking', 'still', 'running', 'walking', 'vehicle', 'walking'];
    exports.activity = Array.from({ length: 6 }, (_, index) => createEventRecord('activity', 'activity_recognition', index, {
  ...
  if (isCollectorReady(collectors, 'appUsage')) {
    exports.app_usage = Array.from({ length: 6 }, (_, index) => createEventRecord('app_usage', 'usage_stats', index, {
  ...
  if (isCollectorReady(collectors, 'deviceState')) {
    const deviceStates = ['charging_on', 'charging_on', 'charging_off', 'charging_off', 'charging_on', 'charging_off'];
    exports.charging_state = Array.from({ length: 6 }, (_, index) => createEventRecord('charging_state', 'system_broadcast', index, {
